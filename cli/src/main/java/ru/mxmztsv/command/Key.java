...
@AllArgsConstructor
public enum Key {
    PROTOCOL(List.of(Command.PROTOCOL), "-p", "--protocol", "-p --protocol\t\t->\tключ в котором нужно указать REST или SOAP"),
    ID(List.of(Command.DELETE, Command.UPDATE), "-id", "--id","-id, --id        \t\t->\tКлюч указывает id клиента"),
